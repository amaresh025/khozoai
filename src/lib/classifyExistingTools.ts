...
/**
 * One-time migration script to classify existing tools
 * that don't have capabilities, industries, or best_for fields populated.
 * Uses keyword matching against tool metadata.
 */
...
const CAPABILITY_KEYWORDS: Record<string, string[]> = {
  ...
  "AI Writing": ["writing", "write", "content", "copywriting", "blog", "article", "grammar", "paraphras"],
  "AI Coding": ["code", "coding", "programming", "developer", "ide", "debug", "autocomplete", "copilot"],
  "AI Research": ["research", "paper", "academic", "scholar", "citation", "literature"],
  "AI Search": ["search", "search engine", "perplexity", "retrieval"],
  "AI Image Generation": ["image", "img", "photo", "art", "midjourney", "dall-e", "stable diffusion", "flux"],
...
export const classifyExistingTools = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: tools, error } = await context.supabase
      .from("tools")
      .select("id, name, slug, short_description, full_description, category_id, use_cases, capabilities, industries, best_for")
      .eq("status", "approved");
