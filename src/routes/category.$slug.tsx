...
import { CAPABILITY_TAXONOMY, Q } from "@/lib/queries";
...
export const Route = createFileRoute("/category/$slug")({
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `The best ${params.slug.replace(/-/g, " ")} AI tools, ranked and reviewed.`,
      },
...
function resolveCapabilityFromSlug(slug: string) {
  const resolved = CAPABILITY_TAXONOMY.find((cap) => slugify(cap) === slug);
  return resolved ?? null;
}

function CategoryPage() {
  const { slug } = Route.useParams();
  console.log("[category.$slug] slug received:", slug);

  const resolvedCapability = resolveCapabilityFromSlug(slug);
  const capabilityName = resolvedCapability ?? prettify(slug);

  console.log("[category.$slug] capability resolved:", {
    slug,
    resolvedCapability,
    capabilityName,
  });
  ...
  const toolsByCapability = useQuery({
    ...
    queryFn: async () => {
      const res = await Q.toolsByCapability(capabilityName);
      const tools = res.data ?? [];
      console.log("[category.$slug] toolsByCapability returned:", {
        slug,
        capabilityName,
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      <Breadcrumbs items={[{ label: "Categories", href: "/categories" }, { label: displayName }]} />
