...
const key = process.env.SUPABASE_PUBLISHABLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxya2Zrc25sbGRjeW10a3ppdXFwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE5MTQxMzUsImV4cCI6MjA5NzQ5MDEzNX0.U07eFqulflRe3tjcJaG2OCZtslyfJK_AMB0nqgz55o4';
...
async function test() {
  ...
  // Test tools
  const { data: tools, error: toolsError } = await supabase.from('tools').select('*').eq('status', 'approved');
  if (toolsError) console.error('Tools error:', toolsError.message);
  else console.log('Approved tools:', tools?.length || 0, 'found');

  // Test blog posts
  const { data: blogs, error: blogError } = await supabase.from('blog_posts').select('*').eq('published', true);
  if (blogError) console.error('Blog posts error:', blogError.message);
  else console.log('Published blog posts:', blogs?.length || 0, 'found');

  // Test prompt templates
  const { data: prompts, error: promptError } = await supabase.from('prompt_templates').select('*');
  if (promptError) console.error('Prompts error:', promptError.message);
  else console.log('Prompt templates:', prompts?.length || 0, 'found');

  // Test comparisons
  const { data: comparisons, error: compError } = await supabase.from('tool_comparisons').select('*');
